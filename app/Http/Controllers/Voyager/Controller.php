...
use TCG\Voyager\Http\Controllers\Controller as MainBaseController;
...
class Controller extends MainBaseController
{
    use DispatchesJobs;
    ...
    public function insertUpdateData($request, $slug, $rows, $data, $uploadedFileUrl = '')
    {
        ...
        $date = $year . '/' . $month;
        $sFiletype = $sPath = ($slug == 'posts') ? 'image' : 'avatar';
        $sPath = ($slug == 'posts') ? 'posts/' . $date :  'avatars/' . $date;

        // If there's no file we dont use the cloudinary API 
        if ($request->$sFiletype) {

            $uploadedFileUrl = Cloudinary::upload(
                $request->file($sFiletype)->getRealPath(),
                [
                    'folder' =>  'farol-azul/' . $sPath
                ]
            )->getSecurePath();
        }
        ...
        if ($slug == 'users') {
            // fixing the role id null Bug
            if ($data->role_id == null) {
                $data->role_id = User::find($data->id)->role_id;
            }
        }

        if ($request->$sFiletype) {
            // Set the cloudinary image path
            if ($slug == 'posts') {
